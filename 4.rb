...
calls.each do |call|
  calls_so_far << call
  if boards.size == 1 && board_winning(boards[0], calls_so_far)
    p boards[0] 
    sum = boards[0].flatten.find_all { |n| !calls_so_far.include? n }.sum
    p sum
    p calls_so_far.last
    p sum * calls_so_far.last
    break
  end
  boards = boards.find_all { |board| !board_winning(board, calls_so_far) }
  p boards.size
end
